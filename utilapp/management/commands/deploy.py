...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        opts = '-uhzravE --exclude="*.pyc" --exclude=manage.py --exclude=settings.py --exclude="mydata.db"'
        if options['test']:
            opts = opts+' -n'
            
        os.system('rsync '+opts+' '+DIR_LOCAL+' '+USER_SERVER+'@'+DOMAIN_SERVER+':'+DIR_SERVER)
        self.stdout.write('\nWARNING: Consider that the file settings.py and manage.py must be sent manually.\n'+ \
        'Don\'t apply any changes in the server directory except into settings.py and manage.py\n')
